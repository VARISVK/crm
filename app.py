...
import psycopg2
import psycopg2.extras # Important for getting dict results
import pandas as pd
...
DB_HOST = "136.112.73.39"  # Public IP of Cloud SQL instance
DB_NAME = "crm_db"         # Make sure this DB exists in your instance
DB_USER = "postgres"
DB_PASS = "YOUR_DB_PASSWORD" # The password you set for 'postgres'

WAHA_URL = "http://localhost:3000"    # WAHA running on the same VM
WAHA_API_KEY = "YOUR_WAHA_API_KEY"    # Key generated by WAHA (check docker logs)
SESSION = "default"                   # Default WAHA session name
...
FIXED_TEMPLATE = "Dear {customer_name}, your {visa_type} visa would be expired on {visa_expiry_date}, kindly renew it."
...
def get_customers():
    ...
    try:
        ...
        cur.execute('SELECT id, customer_name, visa_type, visa_expiry_date::text, country_code, phone_number FROM customers ORDER BY id DESC')
...
def add_customer():
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute(
                """INSERT INTO customers (customer_name, visa_type, visa_expiry_date, country_code, phone_number) VALUES (%s, %s, %s, %s, %s)""",
        ...
        if "unique constraint" in str(e).lower() or "duplicate key value violates unique constraint" in str(e).lower():
        ...
        conn.rollback(); app.logger.error(f"Failed to add customer: {e}")
...
def delete_customer(customer_id):
    ...
    if not conn: return jsonify({"error": "Database connection failed"}), 500
    try:
        with conn.cursor() as cur: cur.execute("DELETE FROM customers WHERE id = %s", (customer_id,))
        conn.commit()
        if cur.rowcount == 0: return jsonify({'success': False, 'error': 'Customer not found'}), 404
        return jsonify({'success': True}), 200
    except Exception as e:
        conn.rollback(); app.logger.error(f"Failed to delete customer {customer_id}: {e}")
...
def get_informed_customers():
    ...
    if filter_by == 'today':
        ...
        query += " WHERE DATE(sent_at AT TIME ZONE 'Asia/Dubai') = %s"
        params.append(today_date_uae)
    ...
    try:
        ...
        for row in cur.fetchall():
            log_dict = dict(row)
            if isinstance(log_dict.get('sent_at'), datetime):
                 log_dict['sent_at'] = log_dict['sent_at'].astimezone(UAE_TZ).strftime('%Y-%m-%d %H:%M:%S %Z')
...
def commit_excel_import():
        ...
        with conn.cursor() as cur:
            for rec_idx, rec in enumerate(data):
                try:
                    ...
                    cur.execute(
                        """INSERT INTO customers (customer_name, visa_type, visa_expiry_date, country_code, phone_number) VALUES (%s, %s, %s, %s, %s) ON CONFLICT (customer_name, visa_expiry_date) DO NOTHING""",
                    ...
                    skipped += 1; conn.rollback(); conn.autocommit = False # Rollback insert only
    ...
    except Exception as e: conn.rollback(); app.logger.error(f"Commit Import Transaction Error: {e}", exc_info=True); return jsonify({"success": False, "error": f"DB transaction error: {e}"}), 500
    finally:
        if conn: conn.close()
...
# 6. Schedule this script using `crontab -e` to run at 10:00 UTC (2 PM UAE).
...
if __name__ == '__main__':
    ...
    # Run using Flask's built-in server for development (debug=True enables auto-reload)
